...
with visualization_tab:
    ...
    with pre_reduction_vis_column:
        ...
        vis_pre_reduction_method = st.selectbox(
            ...
            disabled=not(st.session_state['vis_pre_reduction_check'])
        )
...
with clustering_tab:
    ...
    st.markdown("<h3 style='text-align: center; color: grey;'>Clustering Pipeline</h3>", unsafe_allow_html=True)
    pre_reduction_clus_column, clustering_column = st.columns(2)
    with pre_reduction_clus_column:
        st.markdown('##### Pre-Reduction Options:')
        ...
        clus_pre_reduction_method = st.selectbox(
            ...
            disabled=not(st.session_state['clus_pre_reduction_check'])
        )

        clus_pre_reduction_components = st.slider(
            'Output dimensions', key="clus_pre_reduction_components", step=1, value=5, min_value=4, max_value=32, disabled=not(st.session_state['clus_pre_reduction_check']))

        if clus_pre_reduction_method == "UMAP":
            clus_pre_reduction_n_neighbors = st.slider(
                'Number of neighbors', key="clus_pre_reduction_n_neighbors", step=5, min_value=5, max_value=100, value=15, disabled=not(st.session_state['clus_pre_reduction_check']))
