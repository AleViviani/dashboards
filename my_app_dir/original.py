...
if not(DEV):
    ...
    for experiment in os.listdir(DATA_FOLDER):
        STATIC_IMAGES_PATH = (os.path.join(
            STREAMLIT_STATIC_PATH, experiment, IMAGES_FOLDER))
        ...
        for image in os.listdir(os.path.join(DATA_FOLDER, experiment, IMAGES_FOLDER)):
            # For newer Python.
            shutil.copy(os.path.join(DATA_FOLDER, experiment,
                        IMAGES_FOLDER, image), STATIC_IMAGES_PATH)
            pass

        STATIC_GENERATED_PATH = (os.path.join(
            STREAMLIT_STATIC_PATH, experiment, GENERATED_FOLDER))
        ...
        for image in os.listdir(os.path.join(DATA_FOLDER, experiment, GENERATED_FOLDER)):
            # For newer Python.
            shutil.copy(os.path.join(DATA_FOLDER, experiment,
                        GENERATED_FOLDER, image), STATIC_GENERATED_PATH)
            pass
########################################################
    ...
    if os.path.isdir(os.path.join(DATA_FOLDER, experiment)):
        ...
        with open(os.path.join(DATA_FOLDER, experiment, METADATA_FILE), "r") as file:
            ...
            experiments[experiment]["path"] = os.path.join(
                DATA_FOLDER, experiment, METADATA_FILE)
...
with columns[1]:
    st.write("**Image Size**")
    st.write("{} x {}".format(str(experiments[current_experiment]["image"]["dim"]), str(
        experiments[current_experiment]["image"]["dim"])))
with columns[2]:
    st.write("**Channels #**")
...
with columns[3]:
    ...
    for channel in experiments[current_experiment]["image"]["channels"]["preview"]:
        st.write("{}: {}".format(
            channel, experiments[current_experiment]["image"]["channels"]["preview"][channel]))
with columns[4]:
    st.write("**Model architecture**")
    st.write("{}".format(
        str(experiments[current_experiment]["architecture"]["name"])))
with columns[5]:
    st.write("**Layers #**")
    for idx, filter in enumerate(experiments[current_experiment]["architecture"]["filters"]):
        st.write("{}".format(
            experiments[current_experiment]["architecture"]["filters"][idx]))
with columns[6]:
    st.write("**Latent Dimension**")
    st.write("{}".format(
        experiments[current_experiment]["architecture"]["latent_dim"]))
with columns[7]:
    st.write("**Epochs**")
    st.write("{}".format(
        experiments[current_experiment]["training"]["epochs"]))
with columns[8]:
    st.write("**Batch size**")
    st.write("{}".format(
        experiments[current_experiment]["training"]["batch_size"]))
...
with visualization_column:
    ...
    with st.container():
        visualization_prereduction_check = st.checkbox(
            "Execute Pre-Reduction", value=False, key="prereduction", help=None, on_change=None)
...
with clustering_column:
    ...
    with st.container():
        clustering_prereduction_check = st.checkbox(
            "Execute Pre-Reduction", value=False, key="clustering_prereduction", help=None, on_change=None)
        ...
        if clustering_prereduction_check:
            ...
            clustering_prereduction_components = st.slider(
                'Output dimensions', key="clustering_prereduction_components", step=1, value=5, min_value=4, max_value=32)
            ...
            with st.container():
                if clustering_prereduction_method == "UMAP":
                    ...
                    clustering_prereduction_n_neighbors = st.slider(
                        'Number of neighbors', key="clustering_prereduction_n_neighbors", step=5, min_value=5, max_value=100, value=15)
        ...
        with st.container():
            if clustering_method == "kmeans":
                ...
                min_samples = st.slider(
                    'Min samples', key="min_samples", step=1, min_value=2, max_value=50, value=5)
                clusterer = DBSCAN(
                ...
                min_cluster_size = st.slider(
                    'Min cluster size', key="min_cluster_size", step=1, min_value=2, max_value=50, value=5)
                ...
                n_clusters = st.slider(
                    'Number of clusters', key='n_clusters', step=1, min_value=2, max_value=20, value=5)
...
grid_neighbor = st.slider('Select a range of neighbors',
                          5, 100, (25, 50), key="grid_neighbors_range", step=5)
grid_dist = st.slider('Select a range of distance', 0.0,
                      0.9, (0.3, 0.6), key="grid_dist_range", step=0.1)

compute_button = st.button("Compute", key="Compute",
              help="Compute all the pipeline and visualize")
...
labels = np.array(labels_data['data'])   # Non vengono più usate dopo.... a cosa servono?
...
df_image_paths = pd.DataFrame(
    {
        'image_path': map(
            lambda image: os.path.join(
                current_experiment, IMAGES_FOLDER, image),
            images
        )
    })
...
df_gen_paths = pd.DataFrame(
    {
        'gen_path': map(
            lambda image: os.path.join(
                current_experiment, GENERATED_FOLDER, image),
            images
        )
    })
...
def insert_html(file_html, image = None):
    my_file = codecs.open(file_html, 'r')
    page = my_file.read()
...
if compute_button:
    # Visualization
    if visualization_prereduction_check:
        st.write('Visualization Pre-Reduction: ',
                 visualization_prereduction_method)
        if visualization_prereduction_method == "UMAP":
            reducer = umap.UMAP(n_neighbors=vis_pre_reduction_n_neighbors,
                                min_dist=0, n_components=vis_pre_reduction_components)
    ...
    if visualization_reduction_method == "UMAP":
        reducer = umap.UMAP(n_neighbors=vis_reduction_UMAP_n_neighbors,
                            min_dist=vis_reduction_UMAP_min_distance, n_components=vis_reduction_components)
    elif visualization_reduction_method == "PACMAP":
        reducer = pacmap.PaCMAP(n_components=vis_reduction_components, n_neighbors=vis_reduction_PACMAP_n_neighbors,
                                MN_ratio=vis_reduction_PACMAP_MN_ratio, FP_ratio=vis_reduction_PACMAP_FP_ratio)
    ...
    if clustering_prereduction_check:
        ...
        if clustering_prereduction_method == "UMAP":
            reducer = umap.UMAP(n_neighbors=clustering_prereduction_n_neighbors,
                                min_dist=0, n_components=clustering_prereduction_components)
    ...
    plot_figure.circle('x', 'y', source=datasource, color='color',
                       legend_field='clusters', fill_alpha=0.5, size=12)
    ...
    st.write('neighbors: ', min(grid_neighbor), '  ', max(
        grid_neighbor),  'distances: ', min(grid_dist), '  ', max(grid_dist))
    grid_neighbor_range = [min(grid_neighbor), math.floor(
        (min(grid_neighbor)+max(grid_neighbor))/2), max(grid_neighbor)]
    grid_dist_range = [min(grid_dist), round(
        (min(grid_dist)+max(grid_dist))/2, 2), max(grid_dist)]
    ...
    for nrow, n in enumerate(grid_neighbor_range):
        for ncol, d in enumerate(grid_dist_range):
            ...
            axs[nrow, ncol].scatter(
                reducer[:, 0], reducer[:, 1], c=df_embedding['clusters'], s=10, cmap='Spectral')
